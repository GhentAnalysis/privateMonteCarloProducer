...
def system(command):
  try:    return subprocess.check_output(command, shell=True, stderr=subprocess.STDOUT).decode()
  except: return 'error'
...
def clean(hours):
  for run in glob.glob(os.path.expandvars('/user/$USER/production/*CMSSW*/*')):
