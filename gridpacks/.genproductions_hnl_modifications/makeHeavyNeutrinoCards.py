...
# cc              - charge conjugate for the Dirac model (i.e. replacing n1 with n1~)
#
def makeHeavyNeutrinoCards(mass, coupling, flavours, isPre2017=False, type='trilepton', oneFlavourDecay=False, signFirstFlavor=0, noZ=False, dirac=False, cc=False):
  if signFirstFlavor==1:  sign = '_l1Plus'
  if signFirstFlavor==-1: sign = '_l1Min'
  else:                   sign = ''

  extraOpt =  ('_Dirac' if dirac else '') + ('_cc' if cc else '') + ('_oneFlavorDecay' if oneFlavourDecay else '') + ('_noZ' if noZ else '') + ('_pre2017' if isPre2017 else '') + '_massiveAndCKM' + sign
